...
function SeverityCounts({ scan }: { scan: ScanHistoryItem }) {
  return <div className="flex flex-wrap gap-2 text-xs">
    <Badge variant="critical">C {formatCount(scan.critical)}</Badge>
    <Badge variant="high">H {formatCount(scan.high)}</Badge>
    <Badge variant="medium">M {formatCount(scan.medium)}</Badge>
    <Badge variant="low">L {formatCount(scan.low)}</Badge>
  </div>;
}
